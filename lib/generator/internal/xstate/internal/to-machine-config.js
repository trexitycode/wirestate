 ...
 */
async function toMachineConfig({ machineNode, cache, counter = null }) {
    /**
    ...
    let machineConfig = {
        ...
        invoke: { src: rawstring_1.rawstring(`action('${machineNode.id}')`) }
    };
    ...
    if (machineNode.states.length) {
        const childStateConfigs = await Promise.all(machineNode.states.map(stateNode => {
            return to_state_config_1.toStateConfig({ stateNode, cache, toMachineConfig, counter });
        }));
