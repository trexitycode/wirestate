...
async function xstateConfigGenerator(cache) {
    ...
    const toXstateNode = stateNode => {
        ...
        if (stateNode.useDirective) {
            xstateNode.entry.unshift(rawstring(`assign((ctx) => ({ children: { ...ctx.children, ['${stateNode.id}']: spawn(Machines['${stateNode.useDirective.machineId}']) } }))`));
            xstateNode.exit.unshift(rawstring(`(ctx) => ctx.children['${stateNode.id}'].stop()`));
        }
        return xstateNode;
    };
