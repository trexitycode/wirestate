...
const makeScanner = (tokens, { fileName = '' } = {}) => {
    ...
    const syntaxError = (message = null) => {
        if (token) {
            ...
            return Object.assign(new Error(`SyntaxError${message ? ': ' + message : ''} near [L:${token.line} C:${token.column} File:${fileName}]`), { line: token.line, column: token.column, fileName });
        }
        else {
...
const parseScopeNode = (scanner, { fileName = '' } = {}) => {
    const scopeNode = new ast_nodes_1.ScopeNode(fileName);
...
const parseImportNode = (scanner) => {
    ...
    let importNode = new ast_nodes_1.ImportNode(ids, file);
...
const parseMarchineNode = (scanner) => {
    ...
    while (scanner.token) {
        if (scanner.look({ type: 'indent' })) {
            ...
            let ep = new ast_nodes_1.EventProtocolNode(event);
...
const parseUseDirectiveNode = (scanner) => {
    ...
    let node = new ast_nodes_1.UseDirectiveNode(scanner.consume('identifier').value);
    Object.assign(node, {
        line: typeToken.line,
