...
const makeScanner = (tokens, { wireStateFile = '' } = {}) => {
    ...
    const syntaxError = (message = null) => {
        if (token) {
            ...
            return Object.assign(new Error(`SyntaxError${message ? ': ' + message : ''} near [L:${token.line} C:${token.column} WireState File:${wireStateFile}]`), { line: token.line, column: token.column, wireStateFile });
        }
        else {
